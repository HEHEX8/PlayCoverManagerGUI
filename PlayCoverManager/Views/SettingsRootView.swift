...
private struct DataSettingsView: View {
    ...
    @State private var diskImages: [DiskImageItem] = []
    @State private var isLoadingImages = false
    
    struct DiskImageItem: Identifiable {
        let id = UUID()
        let bundleID: String
        let appName: String
        let imageURL: URL
    ...
    var body: some View {
        ScrollView {
            VStack(spacing: 20) {
                ...
                // Disk Image Resize Card
                VStack(alignment: .leading, spacing: 16) {
                    Label("ディスクイメージ容量管理", systemImage: "arrow.up.and.down.circle")
                        .font(.headline)
                        .foregroundStyle(.purple)
                    ...
                    VStack(alignment: .leading, spacing: 12) {
                        Text("インストール済みアプリのディスクイメージ容量を変更できます。")
                            .font(.callout)
                            .foregroundStyle(.secondary)
                        ...
                        Button {
                            ...
                            Label("ディスクイメージをリサイズ", systemImage: "arrow.up.and.down.circle.fill")
                                .font(.system(size: 14, weight: .medium))
                        }
                        .buttonStyle(.bordered)
                        ...
                        HStack(alignment: .top, spacing: 12) {
                            ...
                            VStack(alignment: .leading, spacing: 4) {
                                ...
                                Text("リサイズを実行する前に、対象アプリを必ずアンマウントしてください。マウント中のイメージはリサイズできません。")
                                    .font(.callout)
                                    .foregroundStyle(.secondary)
                                    .fixedSize(horizontal: false, vertical: true)
                            }
        ...
        .sheet(isPresented: $showResizeSheet) {
            DiskImageResizeSheet(diskImages: $diskImages, isLoading: $isLoadingImages)
...
fileprivate struct DiskImageResizeSheet: View {
    ...
    @Environment(SettingsStore.self) private var settingsStore
    @Environment(AppViewModel.self) private var appViewModel
    @Binding var diskImages: [DataSettingsView.DiskImageItem]
    @Binding var isLoading: Bool
    @State private var isResizing = false
    @State private var resizeStatus = ""
    @State private var resizeProgress: Double = 0
    @State private var resizeResults: [(appName: String, success: Bool, message: String)] = []
    @State private var showResults = false
    ...
    private var selectionView: some View {
        VStack(alignment: .leading, spacing: 12) {
            if diskImages.isEmpty {
                ...
                VStack(alignment: .leading, spacing: 8) {
                    HStack {
                        Text("インストール済みアプリ (\(diskImages.count) 個)")
                            .font(.headline)
                        Spacer()
                    }
                    ...
                    HStack(alignment: .top, spacing: 12) {
                        ...
                        VStack(alignment: .leading, spacing: 4) {
                            Text("マウント中のアプリは変更できません")
                                .font(.subheadline)
                                .fontWeight(.semibold)
                            Text("リサイズを実行する前に、対象アプリをアンマウントしてください。")
                                .font(.caption)
                                .foregroundStyle(.secondary)
                        }
                    }
                    ...
                    ForEach($diskImages) { $item in
                        HStack(spacing: 12) {
                            ...
                            HStack(spacing: 8) {
                                ...
                                TextField("", value: $item.newSizeGB, format: .number)
                                    .textFieldStyle(.roundedBorder)
                                    .frame(width: 70)
                                    .disabled(item.isMounted)
                                ...
                                if item.isMounted {
                                    Image(systemName: "lock.fill")
                                        .foregroundStyle(.orange)
                                        .help("マウント中のため変更できません")
                                }
                            }
                        }
                        .opacity(item.isMounted ? 0.5 : 1.0)
                    }
                }
            }
        }
    ...
    // MARK: - Resizing View
    private var resizingView: some View {
        VStack(spacing: 20) {
            VStack(spacing: 12) {
                ...
                ProgressView(value: resizeProgress)
                    .frame(width: 400)
            }
        }
        .frame(maxWidth: .infinity, maxHeight: .infinity)
    ...
    private var resultsView: some View {
        VStack(spacing: 16) {
            HStack {
                let hasFailures = resizeResults.contains(where: { !$0.success })
                if hasFailures {
                    Image(systemName: "exclamationmark.triangle.fill")
                        .font(.title2)
                        .foregroundStyle(.orange)
            ...
            ScrollView {
                VStack(spacing: 12) {
                    ForEach(resizeResults, id: \.appName) { result in
                        HStack(spacing: 12) {
    ...
    private var bottomButtons: some View {
        HStack {
            ...
            if !isLoading && !isResizing && !showResults && !diskImages.isEmpty {
                let changedImages = diskImages.filter { $0.newSizeGB != $0.currentSizeGB && !$0.isMounted }
                Button("リサイズ実行 (\(changedImages.count) 個)") {
                    Task {
                        await performResize()
                    }
                }
                .buttonStyle(.borderedProminent)
                .disabled(changedImages.isEmpty)
                .keyboardShortcut(.defaultAction)
            }
        }
    }
    ...
    private func loadDiskImages() async {
        isLoading = true
        ...
        do {
            let contents = try fileManager.contentsOfDirectory(at: diskImageDir, includingPropertiesForKeys: [.fileSizeKey])
            var items: [DataSettingsView.DiskImageItem] = []
            
            for url in contents where url.pathExtension == "asif" {
                let bundleID = url.deletingPathExtension().lastPathComponent
                
                // Skip PlayCover container
                if bundleID == "io.playcover.PlayCover" {
                    continue
                }
                ...
                let currentSizeBytes = resourceValues?.fileSize ?? 0
                ...
                items.append(DataSettingsView.DiskImageItem(
                    ...
                    newSizeGB: currentSizeGB,
                    isMounted: isMounted
                ))
            }
            
            await MainActor.run {
                diskImages = items.sorted { $0.appName < $1.appName }
            }
        } catch {
            await MainActor.run {
                diskImages = []
    ...
    private func performResize() async {
        let changedImages = diskImages.filter { $0.newSizeGB != $0.currentSizeGB && !$0.isMounted }
        guard !changedImages.isEmpty else { return }
        ...
        for (index, item) in changedImages.enumerated() {
            await MainActor.run {
                resizeStatus = "\(item.appName) をリサイズ中..."
                resizeProgress = Double(index) / Double(changedImages.count)
            }
            
            do {
                try await diskImageService.resizeDiskImage(for: item.bundleID, newSizeGB: item.newSizeGB)
                await MainActor.run {
                    resizeResults.append((
                        appName: item.appName,
                        success: true,
                        message: "\(item.currentSizeGB) GB → \(item.newSizeGB) GB に変更しました"
                    ))
                }
            } catch {
                await MainActor.run {
