...
final class DiskImageService {
    ...
    /// Find all processes using a disk image volume
    /// - Parameter volumePath: Path to the mounted volume
    /// - Returns: Array of process information (PID, process name, path)
    func findProcessesUsingVolume(at volumePath: URL) async -> [(pid: Int, name: String, path: String)] {
        var processes: [(pid: Int, name: String, path: String)] = []
        
        do {
            // Use lsof to find all open files on the volume
            let output = try await processRunner.run("/usr/sbin/lsof", ["+D", volumePath.path])
            ...
            for line in lines {
                ...
                if let pid = Int(parts[1]) {
                    ...
                    let processPath = getProcessPath(pid: pid) ?? processName
                    
                    // Avoid duplicates
                    if !processes.contains(where: { $0.pid == pid }) {
                        processes.append((pid: pid, name: processName, path: processPath))
                    }
                }
            }
        } catch {
            // lsof may fail if no processes are using the volume (normal)
        }
        
        return processes
    }
    
    /// Get full path for a process by PID
    private func getProcessPath(pid: Int) -> String? {
        do {
            let output = try processRunner.runSync("/bin/ps", ["-p", "\(pid)", "-o", "comm="])
    ...
    /// Get diagnostic information for a mounted disk image
    /// - Parameters:
    ///   - bundleID: Bundle identifier of the app
    ///   - containerURL: Path to the container/mount point
    /// - Returns: Dictionary with diagnostic information
    func getDiskImageDiagnostics(for bundleID: String, containerURL: URL) async -> [String: Any] {
        var diagnostics: [String: Any] = [:]
        
        // Check if mounted
        if let descriptor = try? diskImageDescriptor(for: bundleID, containerURL: containerURL),
           descriptor.isMounted {
            diagnostics["mounted"] = true
            diagnostics["volumePath"] = containerURL.path
            
            // Get device info
            if let infoOutput = try? await processRunner.run("/usr/sbin/diskutil", ["info", "-plist", containerURL.path]),
               let data = infoOutput.data(using: .utf8),
               let plist = try? PropertyListSerialization.propertyList(from: data, options: [], format: nil) as? [String: Any] {
                diagnostics["deviceIdentifier"] = plist["DeviceIdentifier"] as? String
                diagnostics["volumeName"] = plist["VolumeName"] as? String
                diagnostics["filesystemType"] = plist["FilesystemType"] as? String
            }
            
            // Find processes using the volume
            let processes = await findProcessesUsingVolume(at: containerURL)
            if !processes.isEmpty {
                diagnostics["blockingProcesses"] = processes.map { process in
                    [
                        "pid": process.pid,
                        "name": process.name,
                        "path": process.path
